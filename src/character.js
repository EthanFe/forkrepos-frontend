class Character extends GameObject {
  ...
  updateKnockbackStatus() {
    if (
      this.knockbackState.active &&
      new Date().getTime() - this.knockbackState.timeStarted >
        this.knockbackState.duration
    ) {
  ...
  cycleHero() {
    const newIndex =
      this.currentHeroIndex < this.heroesData.length - 1
  ...
  knockBackFrom(source) {
    const knockbackDirection =
      source.x - this.centerPoint.x >= 0 ? "left" : "right";
    this.knockbackState = {
      active: true,
